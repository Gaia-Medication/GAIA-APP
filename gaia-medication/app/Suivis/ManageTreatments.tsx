...
export default function ManageTreatments({ navigation }) {
  ...
  const handleModify = (modifiedTake, newDate) => {
    ...
    const updatedList = takeList.map((take) =>
      take === modifiedTake ? modifiedTake : take
    );
    setTakeList(updatedList);
  };
  ...
  const modalModif = selectedTreatment ? (
    ...
    >
      <Text className=" text-neutral-700 text-xl font-medium">
    ...
    const htmlParts = await Promise.all(
      instructions.map(async (instruction) => {
        return await Promise.all(
          instruction.takes
            .filter((take) => new Date() > new Date(take.date))
            .map(async (take) => {
